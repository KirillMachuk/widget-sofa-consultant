# Виджет-бот для консультации по мебели

Интерактивный виджет-бот для сайта, который помогает клиентам выбрать мебель и собирает лиды в Google Sheets.

## 🚀 Возможности

- **Умный чат-бот** с использованием OpenAI GPT
- **Анализ запросов** - автоматическое определение типа вопроса (FAQ или товары)
- **Формы с подарками** по категориям (Диван, Кровать, Кухня, Другое)
- **Сбор лидов** с автоматической отправкой в Google Sheets
- **Адаптивный дизайн** для мобильных и десктопных устройств
- **Настраиваемые промпты** для консультации по FAQ
- **Интеграция с Google Apps Script** для обработки лидов

## 📋 Требования

- Node.js 22.x или выше
- OpenAI API ключ
- **Upstash Redis** (обязательно) - для хранения сессий и данных виджета
- Google Apps Script для обработки лидов
- Google Sheets для хранения данных

## 🔐 Переменные окружения

Перед запуском создайте файл `.env` на основе `env.example` и заполните обязательные переменные:

- `OPENAI_API_KEY` — ключ для OpenAI
- `GAS_URL` — URL опубликованного Google Apps Script, который пишет лиды в таблицу
- `KV_REST_API_URL` — URL вашего Upstash Redis инстанса (обязательно)
- `KV_REST_API_TOKEN` — токен доступа к Upstash Redis (обязательно)

Файл `.env` не коммитьте в репозиторий. Админ-панель доступна только по прямому пути `/admin`; ссылки на неё не размещаются публично, а индексация запрещена через `robots.txt`.

## 🛠 Установка и развертывание

### Вариант 1: Развертывание на Vercel (Рекомендуется)

1. **Создайте репозиторий на GitHub:**
   ```bash
   git init
   git add .
   git commit -m "Initial commit"
   git branch -M main
   git remote add origin https://github.com/ваш-username/ваш-репозиторий.git
   git push -u origin main
   ```

2. **Подключите к Vercel:**
   - Зайдите на [vercel.com](https://vercel.com)
   - Нажмите "New Project"
   - Импортируйте ваш GitHub репозиторий
   - Настройте переменные окружения:
     - `OPENAI_API_KEY` - ваш ключ OpenAI

3. **Получите URL развертывания** и используйте его для интеграции

### Вариант 2: Развертывание на GitHub Pages

1. **Настройте GitHub Pages:**
   - Перейдите в Settings → Pages
   - Выберите "GitHub Actions" как источник
   - Создайте workflow файл (см. `.github/workflows/deploy.yml`)

2. **Настройте переменные окружения:**
   - Перейдите в Settings → Secrets and variables → Actions
   - Добавьте `OPENAI_API_KEY`

### Вариант 3: Локальное развертывание

1. **Клонируйте репозиторий:**
   ```bash
   git clone https://github.com/ваш-username/ваш-репозиторий.git
   cd ваш-репозиторий
   ```

2. **Установите зависимости:**
   ```bash
   npm install
   ```

3. **Настройте переменные окружения:**
   ```bash
   echo "OPENAI_API_KEY=ваш-ключ-openai" > .env.local
   ```

4. **Запустите локально:**
   ```bash
   npm start
   ```

## 🔧 Интеграция на сайт

### Для Vercel/GitHub Pages:

```html
<script>
  (function(){
    var s=document.createElement('script');
    s.src='https://ваш-домен.vercel.app/widget.js?v=5.1.0';
    s.defer=true;
    s.dataset.prompt='https://ваш-домен.vercel.app/prompt.json';
    s.dataset.gas='https://script.google.com/macros/s/ВАШ-GAS-URL/exec';
    document.head.appendChild(s);
  })();
</script>
```

#### Дополнительные data-атрибуты (опционально)
- `data-avatar` — URL аватарки (по умолчанию используется встроенное изображение)
- `data-avatar-initials` — fallback текст (до 3 символов)
- `data-bottom-offset`, `data-right-offset` — дополнительные отступы, если на сайте есть нижнее меню

### Для локального сервера:

```html
<script>
  (function(){
    var s=document.createElement('script');
    s.src='./widget.js?v=5.1.0';
    s.defer=true;
    s.dataset.prompt='./prompt.json';
    s.dataset.gas='https://script.google.com/macros/s/ВАШ-GAS-URL/exec';
    document.head.appendChild(s);
  })();
</script>
```

## ⚙️ Настройка

### 1. Настройка Google Apps Script

1. Создайте новый проект в [Google Apps Script](https://script.google.com)
2. Замените код на следующий:

```javascript
function doPost(e) {
  try {
    const data = JSON.parse(e.postData.contents);
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Лист1');
    
    // Добавляем заголовки если их нет
    if (sheet.getLastRow() === 0) {
      sheet.getRange(1, 1, 1, 6).setValues([['Имя', 'Телефон', 'Предложение', 'URL страницы', 'Session ID', 'Время']]);
    }
    
    // Добавляем данные
    sheet.appendRow([
      data.name || '',
      data.phone || '',
      data.pretext || '',
      data.page_url || '',
      data.session_id || '',
      new Date(data.timestamp || Date.now())
    ]);
    
    return ContentService.createTextOutput(JSON.stringify({success: true}))
      .setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({error: error.toString()}))
      .setMimeType(ContentService.MimeType.JSON);
  }
}
```

3. Разверните как веб-приложение с доступом "Все пользователи"
4. Скопируйте URL и замените в коде интеграции

### 2. Настройка Google Sheets

Создайте таблицу с колонками:
- **Имя** (A)
- **Телефон** (B) 
- **Предложение** (C)
- **URL страницы** (D)
- **Session ID** (E)
- **Время** (F)

### 3. Настройка виджета

Отредактируйте файлы:
- `prompt.json` - настройки бота и компании (FAQ информация, подарки по категориям)
- `widget.js` (строка 16) - цвета и стиль виджета

## 📁 Структура проекта

```
├── api/
│   ├── chat.js          # API для чата с OpenAI
│   ├── lead.js          # API для отправки лидов
│   └── analytics.js     # API для аналитики
├── images/
│   └── consultant.jpg   # Изображение консультанта
├── index.html           # Тестовая страница
├── widget.js           # Основной код виджета
├── prompt.json         # Настройки бота
├── package.json        # Зависимости проекта
├── vercel.json         # Конфигурация Vercel
└── README.md           # Документация
```

## 🔄 Обновление

При изменении файлов:
1. Обновите версию в `widget.js` (строка 3)
2. Замените `v=5.1.0` в коде интеграции на новую версию
3. Зафиксируйте изменения в Git

## 🐛 Устранение неполадок

### Виджет не появляется:
- Проверьте консоль браузера на ошибки
- Убедитесь, что все URL файлов доступны
- Проверьте настройки CORS

### Лиды не сохраняются:
- Проверьте URL Google Apps Script
- Убедитесь, что Google Sheets имеет лист "Лист1"
- Проверьте права доступа к Google Apps Script

### Ошибки API:
- Проверьте переменную окружения `OPENAI_API_KEY`
- Убедитесь, что у вас есть кредиты на OpenAI

### Ошибки сессий:
- Проверьте переменные окружения `KV_REST_API_URL` и `KV_REST_API_TOKEN`
- Убедитесь, что Upstash Redis инстанс активен
- Проверьте лимиты Upstash (free tier: 10,000 команд/день)

## 💾 Управление сессиями Redis

### Очистка старых сессий

Для очистки всех старых сессий из Redis используйте команду:

```bash
npm run clear-sessions
```

Это удалит все сессии и списки сессий из Redis. Используйте с осторожностью в production!

## 📈 Производительность и масштабирование

### Архитектура хранения данных

- **Redis (Upstash)**: Все сессии хранятся в Redis, обеспечивая стабильную работу на serverless платформе (Vercel)
- **Локальное кэширование промптов**: Промпты кэшируются локально в каждом serverless инстансе на 10 минут для оптимальной производительности
- **TTL сессий**: 30 дней автоматического удаления неактивных сессий

### Оценка нагрузки

Виджет оптимизирован для работы с высокими нагрузками:

- **40,000 посетителей/месяц**: Работает стабильно без дополнительной настройки
- **100,000+ посетителей/месяц**: Может потребоваться мониторинг Redis лимитов
- **Upstash Free tier**: До 10,000 команд/день (примерно 40,000 посетителей/месяц при 10% конверсии в чат)

### Рекомендации по масштабированию

1. **Мониторинг Redis**: Отслеживайте использование команд через Upstash Dashboard
2. **Upgrade Upstash**: При превышении free tier лимитов рассмотрите переход на платный план
3. **Оптимизация**: Промпты кэшируются локально, что снижает нагрузку на Redis

## 📞 Поддержка

При возникновении проблем:
1. Проверьте консоль браузера
2. Убедитесь в правильности всех URL
3. Проверьте настройки Google Apps Script и Sheets

## 📄 Лицензия

MIT License
# GitHub Actions секреты настроены - Sat Oct 25 15:40:12 +03 2025
