(function(){
  ...
  function getWidgetBaseUrl() {
    try {
      ...
      console.warn('Failed to get widget base URL:', e);
    }
    ...
    if (PROMPT && CONFIG.openaiEndpoint) {
      ...
      fetch(initUrl, {
        ...
        if (!res.ok) {
          ...
          trackError('session_init_error', `Session initialization failed with status ${res.status}`, errorDetails);
        } else if (DEBUG) {
        ...
        trackError('session_init_error', `Session initialization failed: ${e.message}`, errorDetails);
...
Какую мебель рассматриваете?`;
  ...
  async function submitLead(name, phone, pretext){
    ...
    try{
      ...
      const leadUrl = CONFIG.openaiEndpoint ? CONFIG.openaiEndpoint.replace('/chat', '/lead') : WIDGET_BASE_URL + 'api/lead';
