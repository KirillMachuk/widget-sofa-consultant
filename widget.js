(function(){
  ...
  els.btn.addEventListener('click', ()=>{
    ...
    if (!widgetOpenedInSession) {
      ...
      addMsg('bot', 'Здравствуйте! Чем могу помочь?');
      // Сохраняем приветственное сообщение в историю
      const history = loadHistory();
      history.push({ role: 'assistant', content: 'Здравствуйте! Чем могу помочь?', ts: nowIso() });
      saveHistory(history);
      ...
      const history = loadHistory();
      const welcomeMessage = history.find(m => m.role === 'assistant' && m.content === 'Здравствуйте! Чем могу помочь?');
      if (welcomeMessage) {
  ...
  function maybeOfferPhoneFlow(botReply){
    ...
    if (matchedTriggers.length > 0 || hasForceWords){
      ...
      // Всегда показываем форму с выбором между скидкой и подушками
      const pretexts = [
      ...
      if (field.type === 'offer') {
        return `
          ...
          <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px">
            <button class="offer-btn" data-offer="10% скидка" style="padding:12px 16px;border:2px solid #e0e0e0;border-radius:12px;background:#fff;cursor:pointer;text-align:left;transition:all 0.2s;min-height:44px;font-size:16px">
              <div style="font-weight:600;color:#333">🎁 10% скидка</div>
              <div style="font-size:12px;color:#666">Скидка на любой диван</div>
            </button>
            <button class="offer-btn" data-offer="2 декоративные подушки" style="padding:12px 16px;border:2px solid #e0e0e0;border-radius:12px;background:#fff;cursor:pointer;text-align:left;transition:all 0.2s;min-height:44px;font-size:16px">
              <div style="font-weight:600;color:#333">🛋️ 2 декоративные подушки</div>
              <div style="font-size:12px;color:#666">В цвет выбранного дивана</div>
            </button>
          </div>
        `;
      }
