...
- `data-avatar="https://nm-shop.by/image/catalog/avatar.jpg"` — собственная аватарка
- `data-avatar-initials="NM"` — fallback, если картинка не загрузилась
- `data-bottom-offset="90"` — поднимает кнопку/подсказку выше нижнего меню
- `data-right-offset="20"` — смещает кнопку от правого края

Добавьте эти строки после `s.dataset.gas = ...` в любом коде интеграции.
