...
function buildSystemPrompt(prompt, locale, aggressiveMode = false, hasContacts = false){
  ...
  if (hasContacts) {
    about += '\n\nКРИТИЧЕСКИ ВАЖНО - КЛИЕНТ УЖЕ ОСТАВИЛ КОНТАКТЫ:\n- Клиент уже заполнил форму или оставил телефон в чате\n- Контакты уже получены, форма уже заполнена - НЕ нужно больше предлагать подарки, форму или просить контакты\n- Вместо этого - мягко поблагодари за заявку (если уместно) и просто консультируй по вопросам клиента\n- НЕ навязывай общение, отвечай только на вопросы клиента\n- Если клиент задает вопросы - отвечай по делу, но не предлагай форму или подарки';
  }
