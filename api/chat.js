...
function buildSystemPrompt(prompt, locale, aggressiveMode = false, hasContacts = false){
  ...
  // Add instructions if client already provided contacts - ВЫСОКИЙ ПРИОРИТЕТ
  if (hasContacts) {
    about += '\n\n⚠️⚠️⚠️ КРИТИЧЕСКИ ВАЖНО - КЛИЕНТ УЖЕ ОСТАВИЛ КОНТАКТЫ ⚠️⚠️⚠️:\n- Клиент уже заполнил форму или оставил телефон в чате\n- Контакты уже получены, форма уже заполнена\n- ЗАПРЕЩЕНО: предлагать подарки, форму или просить контакты\n- Вместо этого: мягко поблагодари за заявку (если уместно) и просто консультируй по вопросам клиента\n- Отвечай только на вопросы клиента, не навязывай общение\n- Если клиент задает вопросы - отвечай по делу, но НИКОГДА не предлагай форму или подарки';
  }
